...
class MarathonPlanTest(TestCase):
    ...
    def test_every_week_has_a_long_run(self):
        plan = MarathonPlan(12)
        self.assertEqual(12, sum(any(s.session_type() == SessionType.Long for s in w.sessions())
                                 for w in plan.week_schedule()))

    def test_every_week_has_a_rest_day(self):
        plan = MarathonPlan(12)
        self.assertEqual(12, sum(7 - len(w.sessions()) for w in plan.week_schedule()))

    def test_every_week_has_an_interval_session(self):
        plan = MarathonPlan(12)
        self.assertEqual(12, sum(any(s.session_type() == SessionType.Interval for s in w.sessions())
                                 for w in plan.week_schedule()))
    ...
    def test_load_peaks_at_the_third_from_last_week(self):
        schedule = MarathonPlan(12).week_schedule()
